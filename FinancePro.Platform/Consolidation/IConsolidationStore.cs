...
public interface IConsolidationStore
{
    ...
    Task<ConsolidationRun?> GetRunAsync(int runId, CancellationToken cancellationToken = default);
    ...
    Task SetRunStatusAsync(int runId, string status, int userId, string userName, CancellationToken cancellationToken = default);
}
